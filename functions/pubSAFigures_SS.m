function pubSAFigures_SS(mT,dt,figFold)
...
% figFold = Name of the subfolder to save in (relative, folder name is suffiecient
    ...
    subInds{fnum} = getSubInds(mT, subset);
    yVars{fnum} = 'filteredHeadEntries';
    ...
    for y = 1:length(yVars)
        if ~isempty(subInds{y})
            plotPubFig(mT, yVars{y}, yLabs{y}, subInds{y}, titles, figNames{y}, figType, donut{y}, ...
...
%%
function [g] = plotPubFig(mT, yVar, yLab, subInd, titles, figName, figType, donut, varargin)
    ...
    set(g(1,3).facet_axes_handles,'YLim',[0 yMax],'XLim',[15 25.5],'XTick',[15 20 25],'XTickLabel',{'0','5','10'});
