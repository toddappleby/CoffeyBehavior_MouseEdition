function dailySAFigures_SS(mT,dt,figFold)
    ...
    yVals = {'ActiveLever', 'InactiveLever', 'EarnedInfusions', 'Intake', 'filteredHeadEntries', 'lastLatency' };
    ...
    for f = 1:length(figName)
        if ~isempty(figName{f})
            plotDailies(mT, yVals, yLabs, figName{f}, 'GrammOptions', grammOptions{f}, 'OrderOptions', orderOptions{f}, 'LegOptions', legOptions{f});
        end
    end
    
end

function plotDailies(mT, yVals, yLabs, figName, varargin)
    p = inputParser;
    ...
    for y = 1:length(yVals)
        ...
        g(row,col).axe_property('LineWidth',1.5,'XLim',[0 30],'TickDir','out');
