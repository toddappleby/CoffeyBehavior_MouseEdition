...
masterTable_flnm = '.\27-Jan-2025_masterTable.mat';
...
sessionDateType_flnm = '.\Session Date Key.xlsx';
...
firstHour = false; % separately run the first-hour of data (in addition to the full session)
...
% import session date type sheet
sdtKey = readtable(sessionDateType_flnm);

% SS Note: Get from sessDateType instead? or will that screw up hopes for SA? 
...
% pull specific run and experiment type to analyze
mT_tags = getMTtags(runNum, runType, sessTypes, mKey);
mT_ind = find(ismember(mT.TagNumber, mT_tags));
mT = mT(mT_ind,:);

sdt_inds = find((sdtKey.Run == runNum) .* strcmp(sdtKey.Experiment,runType));
sdtKey = sdtKey(sdt_inds,:);
...
if strcmp(runType, 'BE')
    ...
    BE_sess = unique(sdtKey.Session(strcmp(sdtKey.SessionType,'BehavioralEconomics')));
...
for i=1:height(mTDL)
    ...
    % % SSnote: hard-coded drug intake, not ideal
